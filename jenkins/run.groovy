pipeline {
    ...
    parameters {
        ...
        string(name: "QUEUE", defaultValue: "q.request.test", description: "Change this if you need to run on a different queue")
    }
    stages {
        stage("Run Suite") {
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'rabbit-credentials', passwordVariable: 'RABBIT_PASS', usernameVariable: 'RABBIT_USER')]) {
                        sh "docker run -e RABBITMQ_DEFAULT_USER=$RABBIT_USER -e RABBITMQ_DEFAULT_PASS=$RABBIT_PASS radusimonica/test_distributor:$DOCKER_TAG java -jar /test-distributor/distributedTest.jar --publisher --suite $SUITE --rabbitHost 192.168.0.150 --requestQueue $QUEUE"
                    }
                }
            }
        }
